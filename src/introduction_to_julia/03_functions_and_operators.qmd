...
ㅋ
- `x//y` 는 `x`, `y` 가 정수일 때 Julia 의 `Rational` 타입의 값을 반환합니다. 앞서 말했듯이 Julia 에서 유리수는 `Int64` 와 같은 고유한 타입입니다.
...
불(Bool) 타입은 `true`(참) 와 `false`(거짓) 의 두가지 값을 가지는 정수형 타입으로, `true` 는 `1` 과 같고 `false` 는 `0` 과 같습니다. 다른 정수는 불 타입으로 변환되지 않으며 변환시킬 경우 에러가 발생합니다.
...
익명함수가 실제로 널리 사용되는 부분은 [`map` 함수에서의 익명함수의 사용](04_containers2.qmd#sec-anonymous_function_in_map) 에서 볼 수 있듯이 `map`, `reduce`, 등에서 유용하게 사용됩니다. 또한 [`배열 comprehension`](06_arrays.qmd#sec-array_comprehension) 에서도 널리 사용됩니다. 
...
람다 함수 이외에 익명함수를 정의하는 방법은 `do ... end` 블럭과 `begin ... end` 블럭이 있습니다. 이것은 [`do ... end`, `begin ... end` 의 사용](04_containers2.qmd#sec_usage_of_do_end_begin_end) 에서 좀 더 다루도록 하겠습니다.
...
실제로 코드상에서 기본값이 없는 선택적 인자는 없습니다. 일반적으로 기본값이 없는 선택적 인자는 인자의 값이 주어졌느냐 안주어졌느냐가 중요 할 때 사용합니다. 예를 들어 위의 `myadd3` 에서 `myadd(2, 1)` 과 `myadd(2)` 는 구별 할 수 없습니다. 인자가 선택되지 않았을 때 어떤 기능을 하고싶다면 어떻게 할까요? 특별히 인자가 특정 타입을 받는 경우라면? 이때는 보통 `nothing` 을 기본 인자로 놓습니다. 이 선택적 인자에 값을 입력한다면 정수여야 한다면 타입 유니온
...
함수를 정의 할 때 인자의 타입이나 리턴값의 타입을 `::` 연산자를 통해 지정 할 수 있습니다. 예를 들어,
...
::: {.callout-caution}
위의 출력에서 `myfunc2(1)` 의 경우는 긴 에러메시지가 출력되지만 생략했습니다. 앞으로도 긴 에러메시지의 경우, 특별히 중요한 에러메시지가 아니면 생략하도록 하겠습니다.
:::
...
함수 ₩름과 인자의 갯수, 각 인자의 타입의 정보를 함수의 시그너쳐(signature)라고 합니다. Julia 에서는 하나의 함수 이름으로 인자의 타입과 갯수에 따라 각각 다른 기능을 하게 할 수 있는데 이를 메서드(method)라고 합니다. 자세한 것은 [메서드 다중 디스패치](08_type_hierarchy.qmd#sec-method_multiple_dispatch) 에서 다루도록 하겠습니다.
