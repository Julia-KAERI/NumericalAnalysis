...
새로운 패키지를 개발한다면 저는 다음의 과정을 따릅니다. (저는 이미 github 계정이 있습니다.)

1. github 에 리포지터리를 생성합니다. 리포지터리 이름은 당신이 만들고자 하는 패키지 이름 + ".jl" 로 합니다. 예를 들어 `MyAwesomePackage.jl` 같은 이름을 사용 할 수 있습니다. 여기서는 `NAJ.jl` 로 합니다. 간단하게 `readme.md` 파일을 작성할 수도 있습니다.
2. `git clone` 을 통해 리포지터리를 복제합니다. 당연히 별거 없습니다. 별도의 이름을 지정하지 않았다면 `NAJ.jl` 디렉토리가 생겼을 것입니다. 
3. 환경을 설정합니다.
...
이제 터미널에서 `NAJ.jl` 디렉토로 들어갑니다. 그리고 julia REPL 을 실행시키고 `]` 키를 통해 pkg 모드에 진입합니다.

```txt
(@v1.10) pkg>
```

와 같은 프롬프트가 보일 것입니다. `activate .` 명령은 현재 디렉토리의 환경을 활성화 하라는 명령입니다. 
...
현재까지 당신의 `NAJ.jl` 디렉토리는 github 에서 사용하는 `.git` 디렉토리를 제외하면 비어있습니다.

```txt
In [4]: readdir()
Out[4]: 1-element Vector{String}:
 ".git"
```
...
`Manifest.toml` 파일과 `Project.toml` 파일이 생겼습니다. 그리고 `Pkg.status()` 함수를 통해 `Example` 패키지가 설치되었다는 것을 알게 되었습니다. [`toml`](https://toml.io/ko/v1.0.0) 은 설정을 간편하게 읽고 쓰게 해주는 파일 포멧입이며 보통 `.toml` 확장자를 갖습니다. 텍스트 형식으로 저장하기 때문에 문서편집기에서 직접 편집 할 수 있습니다. 두 파일에 대해 간략히 요약하면 다음과 같습니다.
...
`Project.toml` 과 `Manifest.toml` 쌍이 주어지면 정확히 동일한 패키지 환경을 인스턴스화할 수 있으며 이는 재현성에 매우 유용합니다. 자세한 내용은 [`Pkg.instantiate`](https://pkgdocs.julialang.org/v1/api/#Pkg.instantiate)를 참조하기 바랍니다.
...
이제 개발하고자 하는 패키지의 UUID 가 필요합니다. Julia REPL 에서 `using UUIDs;uuid4()` 를 통해 생성 할 수 있습니다. UUID 는 고유식별자이므로 중복되서는 안됩니다.

여기에 앞부분에 아래와 같이 추가합시다.

```toml
name = "NAJ.jl"
uuid = "7876af07-990d-54b4-ab0e-23690620f79a" #실제 uuid 로 변경해야함.
authors = ["myemail <account@myemail.address.com"] # 실제 이름과 이메일
version = "0.1.1"
...
`name` 은 패키지 이름, `authors` 는 저자들 정보, `versions` 는 버젼 정보입니다. 버젼 정보는 `.`으로 이어진 세개의 수로 구성되며 보통 주버젼, 부버젼, 패치버젼을 말합니다. 보통 정식으로 배포되기전에는 주버젼을 `0` 으로 합니다. 
...
일반적으로 소스코드는 패키지 디렉토리의 `/src` 하부 디렉토리에 저장됩니다. 
