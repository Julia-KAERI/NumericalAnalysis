...
::: {.callout-important}
비록 영어로 되어 있긴 하지만 Julia 의 기본 도움말은 매우 훌륭하다. Julia REPL 에서는 `julia> ` 프롬프트에 `?` 를 치면 `help> `로 변경되는 것을 볼 수 있을 것이다. 여기에 함수 이름이라든가, 타입 이름 같은 것을 치면 그에 대한 도움말을 제공한다. Jupyter 에서는 입력 셀에 `?sin` 과 같이 `?` 를 맨 앞에 두고 입력한후 실행하면 문서를 찾아 제공한다. vscode 에서는 자동적으로 팝업 윈도우로 제공한다. 이렇게 제공되는 문서는 당신이 사용하는 Julia 와 페키지에 대한 공식 문서이므로, 가장 믿을만하고 우선적으로 참고해야하는 문서이다.
:::
...
Julia 에서는 수학 기호를 포함한 다양한 기호를 입력하고 사용 할 수 있다. 보통 `\` 로 시작되는 구문을 입력한 후에 공백 없이 탭 키를 누르면 자동적으로 기호로 변환된다. 입력구문의 경우 LaTeX 와 같거나 유사한 경우가 많다. 예를 들어 Julia REPL(Julia 에서 지원하는 interactive command line 환경) 이나 jupyter notebook 에서 `\alpha` 를 치고 곧바로 탭(tab) 키를 누르면 `α` 로 변환된다. 다음과 같은 것도 가능하다.
...
0.0
```
그리스 기호 `π` 는 `\pi` 를 친 후 공백 없이 탭키를 누르면 된다. julia 에서 `pi` 와 `π` 는 모두 원주율 $\pi$ 를 의미한다. 오일러 상수 $e \approx 2.718281$ 는 `\euler` + 탭키로 입력 할 수 있다. `cos(θ)` 는 `θ` 에 대한 코사인함수값이지만 `cosθ` 는 변수명일 뿐이다. Julia 에서는 이런 기호를 변수명 등에 자주 사용한다. 자세한 내용은 [Julia Unicode Input](https://docs.julialang.org/en/v1/manual/unicode-input/#Unicode-Input) 을 참고하라. 
...
Julia 에서 기본적으로 제공하는 수 타입은 다음과 같다.
...
- 앞서 언급한 `π` ($\pi$) 나 `ℯ` ($e$) 는 무리수 타입으로, 연산할 때 가장 적절한 타입으로 변환되어 계산된다. 
- 타입 이름은 다른 타입의 값을 타입으로 변환시키는 함수이기도 하다. `Float64(1)` 은 `1` 을 `Float64` 타입으로 변환시킨다.
- 실제 Julia 의 타입 구조는 이보다 훨씬 복잡하다. 이후 @sec-type_hierarchy 에서 좀 더 자세히 설명하기로 한다.
