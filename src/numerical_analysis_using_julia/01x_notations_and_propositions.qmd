...
$\boldsymbol{A}\in \mathcal{M}_{n \times n}(\mathbb{R})$ 일 때 $\boldsymbol{A}^T= \boldsymbol{A}$ 인 행렬을 대칭 행렬이라고 한다. 대칭 행렬은 정규행렬이다. 대칭 행렬에 대해 다음이 성립한다.

&emsp; ($2$) 서로 다른 고유값을 갖는 에르미트 행렬의 고유벡터는 직교한다.

&emsp; ($3$) 에르미트 행렬의 고유벡터로 정규직교기저를 구성 할 수 있다.

&emsp; ($4$) 정규직교행렬은 직교 변환에 의해 대각행렬이다. 이 대각행렬의 대각성분이 고유값이다.
...
구간 $[a,\,b]$ 에서 정의된 함수 $f$ 를 생각하자. 수열 $\langle x_n\rangle $ 는 $x_1=a<x_2<\cdots <x_n=b$ 를 만족하는 수열 일 때,
