...
이므로 우리는 이것을 이용하여 1차 뿐만 아니라 2차 도함수에 대한 근사값도 구할 수 있다. 여기서 $f^{(3)}(\xi_2) +f^{(3)}(\xi_1)$ 를 보자. $\xi_1\in [x-h,\,x],\, \xi_2 \in [x,\,x+h]$ 인데, $f\in C^3_{[x-h,x+h]}$ 이면 [따름정리 : 여러 점의 경우](01x_notations_and_propositions.qmd#prp-intermediate_value_for_many_points) 에 의해 $f^{(3)}(\xi) = \frac{1}{2}\left(f^{(3)}(\xi_2) +f^{(3)}(\xi_1) \right)$ 를 만족하는 $\xi\in[x-h,\,x+h]$ 가 존재하며, 같은 이유로 $f\in C^4_{[x-h,\,x+h]}$ 이면 $f^{(4)}(\xi')=\frac{1}{2}\left(f^{(4)}(\xi'_1) +f^{(4)}(\xi'_2) \right)$ 를 만족하는 $\xi'\in [x-h,\,x+h]$ 가 항상 존재한다. 따라서 다음과 같이 도함수와 2차도함수를 근사 할 수 있다.
...
이다. $f\in C^2_{[a,\,b]}$ 라면  [따름정리 : 여러 점의 경우](01x_notations_and_propositions.qmd#prp-intermediate_value_for_many_points) 에 의해 $\displaystyle \sum_{i=1}^{n-1}f^{(2)}(\xi_i) = (n-1)f^{(2)}(\xi)$ 를 만족하는 $\xi\in [a,\,b]$ 가 존재하므로, 
...
앞서 사다리꼴 방법을 통해 $O(h^2)$ 의 오차를 갖는 함수 $f$ 의 적분값을 얻었다. 이것을 [짝수승 오차의 리차드슨 외삽법](05_calculus_of_one_variable_function.qmd#sec-richardson_extrapolation_with_even_order_error)과 결합하면 $O(h^4)$ 의 오차를 갖는 적분값을 계산 할 수 있을 것이다. 
...
($1$) 우선 $p(x)$ 의 차수가 $n$ 보다 작은 경우를 보자. $p(x)$ 를 [라그랑쥬 다항식](04_interpolation.qmd#sec-interpolation_lagrange)에 대해 표현하면
