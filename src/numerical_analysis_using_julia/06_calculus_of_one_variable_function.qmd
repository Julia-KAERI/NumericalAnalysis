...
$$
\dfrac{\partial z^{(m)}_i}{\partial x_j} = \sum_{k_1}\cdots \sum_{k_{m-1}}\dfrac{\partial z^{(1)}(\bf{a})}{\partial x_j} \cdots \dfrac{\partial z^{(m-1)}(\bf{z}^{(m-2)})}{\partial z^{(m-1)}_{k_{m-2}}}\dfrac{\partial z^{(m)}(\bf{z}^{(m-1)})}{\partial z^{(m)}_{k_{m-1}}}
...
$$
F^{(1)}(x)= \begin{bmatrix} x \\ x^2\end{bmatrix},\, F^{(2)}(\bf{x})=\begin{bmatrix} x_1 \\ e^{x_1+2x_2}\end{bmatrix}
$$

에 대해 $F:\R \to \R^2$ 가 $F=F^{(2)}\circ F^{(1)}$ 으로 주어졌다고 하자. 

$$
\bf{J}_{F^{(1)}}(\bf{x})= \begin{bmatrix} 1 \\ 2x \end{bmatrix},\, \bf{J}_{F^{(1)}} = \begin{bmatrix} 1 & 0 \\ e^{x_1+2x_2} & 2e^{x_1+2x_2} \end{bmatrix}
$$

이다. 이 때 $\bf{J}_{F}(a)$ 를 두가지 방법으로 구해보자.

우선 전진법으로는 $\bf{J}_{F^{(1)}}(a)$ 와 $\bf{z}^{(1)}=F^{(1)}(a)$ 을 구하고 난 뒤 $\bf{J}_{F^{(2)}}(\bf{z}^{(1)})$ 을 구하고, $\bf{J}_F(\bf{a}) = \bf{J}_{F^{(2)}}(\bf{z}^{(1)})  \bf{J}_{F^{(1)}}(\bf{a})$ 를 계산한다. 많은 함수의 합성의 경우 즉 $F=F^{(N)} \circ \cdots \circ F^{(1)}$ 과 같은 경우에도 전진법은 
