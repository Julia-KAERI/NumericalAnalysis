...
$$
\begin{aligned}
\dfrac{\partial z^{(m)}_i}{\partial x_j} &= \sum_{k_1}\cdots \sum_{k_{m-1}}\dfrac{\partial z_{k_1}^{(1)}(\bf{a})}{\partial x_j} \cdots \dfrac{\partial z_{k_{m-1}}^{(m-1)}(\bf{z}^{(m-2)})}{\partial z^{(m-2)}_{k_{m-2}}}\dfrac{\partial z_i^{(m)}(\bf{z}^{(m-1)})}{\partial z^{(m)}_{k_{m-1}}} \\[0.3em]
&=\sum_{k_{m-1}} \cdots \sum_{k_1}  \dfrac{\partial z_i^{(m)}(\bf{z}^{(m-1)})}{\partial z^{(m)}_{k_{m-1}}} \dfrac{\partial z_{k_{m-1}}^{(m-1)}(\bf{z}^{(m-2)})}{\partial z^{(m-2)}_{k_{m-2}}} \cdots \dfrac{\partial z_{k_1}^{(1)}(\bf{a})}{\partial x_j} \\[0.3em]\\
&= \left[\bf{J}_{F^{(m)}}(\bf{z}^{(m-1)}) \cdots \bf{J}_{F^{(1)}}(\bf{a})\right]_{ij}
\end{aligned}
$$ {#eq-master_eq_of_forward_autodiff}

$m=1$, 즉 $\dfrac{\partial z^{(1)}(\bf{a})}{\partial x_j}$ 부터 계산하기 시작하여 $m=2,\,3,\ldots,\,N$ 까지 증가시키면 $\bf{J}_F$ 를 계산 할 수 있다. 
...
$$
\begin{aligned}
\dfrac{\partial y_j}{\partial z^{(m)}_i} &= \sum_{k_{N-1}}\cdots \sum_{k_{m+1}} \dfrac{\partial y_j(\bf{z}^{(N-1)})}{\partial z^{(N-1)}_{k_{N-1}}} \dfrac{\partial z^{(N-1)}_{k_{N-1}}(\bf{z}^{(N-2)})}{\partial z^{(N-2)}_{k_{N-2}}} \cdots \dfrac{\partial z^{(m+1)}_{k_{m+1}}(\bf{z}^{m})}{\partial z^{(m)}_i} \\[0.3em]
&=\sum_{k_{m+1}} \cdots\sum_{k_{N-1}} \dfrac{\partial z^{(m+1)}_{k_{m+1}}(\bf{z}^{m})}{\partial z^{(m)}_i} \cdots  \dfrac{\partial z^{(N-1)}_{k_{N-1}}(\bf{z}^{(N-2)})}{\partial z^{(N-2)}_{k_{N-2}}}\dfrac{\partial y_j(\bf{z}^{(N-1)})}{\partial z^{(N-1)}_{k_{N-1}}} \\[0.3em]
&= \left[\bf{J}_{F^{(m+1)}}(\bf{z}^m)^T \cdots \bf{J}_{F^{(N-1)}}(\bf{z^{(N-1)}})^T \right]_{ij}
\end{aligned}
...
즉 후진법은 전진법에 비해 과정이 더 길고 함수값을 기억하고 있어야 한다는 단점이 있다. 일반적으로 $F:\R^n \to \R^m$ 에 대해 $m>n$ 이면 전진법이 유리하며 $n>m$ 이면 후진법이 유리하다는 것이 잘 알려져 있다. $n$ 이 $m$ 보다 많이 크다면 후진법이 훨씬 좋다는 것이 알려져 있으며, 기계학습이 전형적으로 이런 경우이므로 기계학습에서는 거의 후진법이 사용된다.
...
::: {#exm-forward_reverse_mode_comparision}

#### 전진법과 후진법

$F:\R^3 \to \R$ 인 $F(\bf{x}) = x_1^2 + e^{x_2 + 2x_3}$ 에 대해 $\bf{a} = (1, 0, 1)$ 에서의 미분을 전진법과 후진법으로 구해보자. 
