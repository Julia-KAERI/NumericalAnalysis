...
로 풀 수 있다. 이것이 잠시 후 다를 오일러 방법이다. 우리는 앞서 [리차드슨 외삽법](05_calculus_of_one_variable_function.qmd#sec-richardson_extrapolation) 에서 보았던 것처럼 주변의 복수의 값을 이용하여 정밀도를 높일 수 있다. 즉 $F(t_i, f(t_i))$ 뿐만 아니라 몇몇 $\alpha_j$ 값에 대해 $F(t_i, f(t_i + \alpha_j h))$ 를 선형결합하여 
