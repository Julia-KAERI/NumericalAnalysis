...
상미분 방정식은 하나의 독립변수에 대한 미분방정식이다. $f(x)$ 에 대한 미분방정식이 $f$ 에 대해 최대 $n$ 차 미분까지 포함되었을 경우, $n$-차 상미분 방정식이라 한다. 상미분 방정식 자체로는 어떤 미지수를 가질 수 밖에 없으며, 이 미지수를 어떤 $x_0$ 에 대한 $f(x_0)$ 값과 $f^{(i)}(x_0)$, $i=1,\,2,\ldots$ 값을 통해 알아내는 것을 **초기값 문제 (initial value problem)** 이라고 한다. 
...
를 이용하여 $\boldsymbol{x}(t_{k+1})$ 값을 구한다. 위의 식에서 $\boldsymbol{f}$ 가 $t$ 만의 함수일 때 아래와 같이 julia 로 구현 할 수 있다.
...
&= x_i(t)  + f(t, \boldsymbol{x})\, h + \dfrac{1}{2} \left(\dfrac{\partial f}{\partial t} + \sum_j f_j(t,\,\boldsymbol{x}) \dfrac{\partial f}{\partial x_j}\right) h^2 + \cdots
\end{aligned}
$$
