...
$\mathbb{R}^n$ 에서의 열린 집합 $U$ 에서 정의된 함수 $\bf{F}:U \to \mathbb{R}^m$ 이 

$$
\lim_{\bf{h} \to \bf{0}} \|\bf{F}(\bf{p} + \bf{h}) - \bf{F}(\bf{p})\|  = 0
$$

일 때 $\bf{F}$ 는 $\bf{p}$ 에서 **연속** 이라고 한다. $\bf{F}$ 가 모든 $\bf{p}\in U$ 에서 연속일 때 $\bf{F}$ 를 $U$ 에서 연속이라고 한다.
...
$\mathbb{R}^n$ 에서의 열린 집합 $U$ 에서 정의된 함수 $\bf{F}:U \to \mathbb{R}^m$ 이 $\bf{p}\in U$ 에서 어떤 행렬 $M\in \R^{m \times n}$ 에 대해

$$
\lim_{\bf{h} \to \bf{0}} \dfrac{\|\bf{F}(\bf{p} + \bf{h}) - \bf{F}(\bf{p}) - \bf{Mp}\|}{\|\bf{h}\|} = \bf{0}
$$

를 만족 할 때 $\bf{F}$ 는 $\bf{p}$ 에서 미분가능하다 라고 한다. 이때 $\bf{M}$ 을 $\bf{p}$ 에서의 $\bf{F}$ 의 **자코비 행렬(Jacobian matrix)** 이라고 하며 $D\bf{F}(\bf{p}),\, \bf{J}_\bf{F}(\bf{p})$ 옥은 $\bf{F}'(\bf{p})$ 라고 표기한다. $\bf{F}$ 가 모든 $\bf{p}\in U$ 에서 미분 가능일 때 $\bf{F}$ 를 $U$ 에서 미분가능하다 라고 한다.
...
$\mathbb{R}^n$ 에서의 열린 집합 $U$ 에서 정의된 함수 $\bf{F}:U \to \mathbb{R}^m$ 에 대해 다음은 동치이다.

&emsp; ($1$) $\bf{F}$ 는 $\bf{p}\in U$ 에서 미분가능하다.

&emsp; ($2$) 어떤 $\bf{M}\in \R^{m \times n}$ 과 $\displaystyle \lim_{\bf{h}\to \bf{0}}\dfrac{\bf{R}(\bf{h})}{\|\bf{h}\|} = \bf{0}$ 인 어떤 함수 $\bf{R}:U \to \mathbb{R}^m$ 에 대해 
$$
\bf{F}(\bf{p}+\bf{h}) = \bf{Mp}+\bf{R}(\bf{v})
$$ 
&emsp; &emsp; 이다. 
...
$\bf{F} : U\subset \mathbb{R}^n \to \mathbb{R}^m$ 가 $\bf{p}\in U$ 에서 미분가능할 때 임의의 $\bf{u}\in \mathbb{R}^n-\{\bf{0}\}$ 에 대해 아래의 극한이 존재한다.

$$
\lim_{t \to \infty} \dfrac{\bf{F}(\bf{p}+t\bf{u}) - \bf{F}(\bf{p})}{t}
...
#### 방향미분과 편미분
$\bf{u}$ 가 단위벡터, 즉 $\|\bf{u}\|=1$ 일 때 @eq-directional_derivative 의  극한을 $\bf{F}$ 의 $\bf{p}$ 에서의 $\bf{u}$ 방향으로의 **방향미분 (directional derivative)** 라고 하고 $D_\bf{u}\bf{F}(\bf{p})$ 라고 쓴다. $\bf{u}$ 가 어떤 표준기저 $\hat{\bf{e}}_i$ 일 때의 방향미분을 **편미분 (partial derivative)** 라고 하며 $D_{\hat{\bf{e}}_i} \bf{F}(\bf{p})$ 를 $D_i \bf{F}(\bf{p})$, $\dfrac{\partial \bf{F}}{\partial x_i}(\bf{p})$, 혹은 $\partial_i \bf{F}(\bf{p})$ 라고 표기한다. 
...
$\bf{F} : U \subset \mathbb{R}^n \to \mathbb{R}^m$ 이 $\bf{p}\in U$ 에서 미분가능이면 $\bf{p}$ 에서 연속이다.
