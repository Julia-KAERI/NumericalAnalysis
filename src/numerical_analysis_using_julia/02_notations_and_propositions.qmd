...
$n$ 차원 유클리드 공간 $\R^n = \{(x_1,\ldots,\,x_n): x_i\in \R,\,i=1,\ldots,\,\}$ 을 $(a_1,\ldots,\,a_n)$ 형식으로 표현 하기도 하고 열벡터 형식, 즉 $\begin{bmatrix} a_1 &\ldots &a_n\end{bmatrix}^T$ 형식으로 로 표기하기로 하자. 즉 유클리드 공간상의 점으로서 사용될 때 두 표기는 동등하다.
...
실수 성분을 갖는 $m \times n$ 행렬 전체의 집합을 $\R^{m \times n}$ 이라고 한다. 복소수 성분을 갖는 $m \times n$ 행렬 전체의 집합을 $\C^{m \times n}$ 라고 하며, 실수/복소수 여부가 일단 고정되기만 하면 상관 없는 경우 $\F^{m \times n}$ 라고 표기한다. 행렬 $\bf{A}$ 의 $i$ 행 $j$ 열 성분은 $(\bf{A})_{ij}$ 혹은 $A_{ij}$ 와 같이 표기한다.

$n\times 1$ 행렬의 집합을 특히 $\F^n$ 라고 한다. 실수/복소수 여부가 정해져 있을 경우 $\mathbb{F}$ 는 $\mathbb{R}$ 이나 $\mathbb{C}$ 가 된다. $\bf{v}\in \F^n$ 의 $j$ 행 성분은 $v_j$ 로 표기한다. $\bf{A}\in \F^{m \times n}$ 에 대해 $B_{ij} = A_{ji}$ 인 행렬 $\bf{B}\in \F^{n \times n}$ 를 $\bf{A}$ 의 **전치 행렬 (transposed matrix)** 이라고 하며 $\bf{A}^T$ 라고 쓴다. $\bf{A}=\bf{A}^T$ 일 때 $\bf{A}$ 를 **대칭 행렬(symmetric matrix)** 이라고 한다. $\bf{A}$ 가 복소행렬일 때 $C_{ij} = \overline{A_{ji}}$ 인 행렬 $\bf{C}$ 를 **수반 행렬 (adjoint matrix)** 라고 하며 $\bf{A}^\ast$ 라고 쓴다. $\bf{A}=\bf{A}^T$ 이면 **대칭 행렬** 이라고 하고 $\bf{A}=\bf{A}^\ast$ 이면 **에르미트 행렬(Hermitian matrix)** 이라고 한다.
...
이미 사용했지만 행렬은 $\bf{A}$ 와 같이 기울어진 굵은 글씨체 대문자로 표현한다. 열벡터, 즉 벡터의 행렬표현의 경우 $\bf{v}$ 와 같이 기울어진  굵은 글씨체의 소문자로 표현한다. 집합의 경우는 $X, A$ 와 같이 대문자를, 집합의 원소의 경우는 소문자를 사용한다. 학문적인 관례에 따르거나, 혹은 특별한 필요가 있을 경우 언급과 함께 이 원칙에 어긋날 수 있다.
...
어떤 집합 $V$ 와 체 $\F$ 에 대해 $V$ 의 원소 사이의 덧셈과 $\F$ 와 $V$ 사이의 곱셈이 정의되어 있으며, 다음이 성립하면 $V$ 를 **$\mathbb{F}$ 위에서의 벡터공간** 혹은 **$\mathbb{F}$-벡터공간** 이라 한다.
...
벡터공간의 기저는 항상 찾을 수 있다. 또한 벡터공간의 기저는 유일하지 않다. 그러나 유한차원 벡터공간의 기저에 포함되는 벡터의 갯수는 정해져 있으며 이 갯수를 벡터공간의 **차원(dimension)** 이라고 한다. 벡터공간 $V$ 의 차원은 $\dim (V)$ 로 표기한다. $\dim(\F^n) = n$ 이다.
...
($2$) $n$ 차원 $\mathbb{F}$-벡터공간과 $\F^n$ 는 동형이다. 

($3$) $n$ 차원 $\mathbb{F}$-벡터공간 $U$ 와 $m$ 차원 $\mathbb{F}$-벡터공간 $V$ 사이의 선형사상의 집합 $\mathcal{L}(U, V)$ 와 $\F^{m \times n}$ 는 동형이다.
...
$\bf{A}\in \F^{m \times n}$ 에 대해 $\ker (\bf{A})$, $\text{im}\, (\bf{A})$, $\text{rank}\, (\bf{A})$, $\text{nuliity}(\bf{A})$ 는 다음과 같이 정의된다.

$$
\begin{aligned}
\ker (\bf{A}) &= \{\bf{v}\in \R^n : \bf{Av}=\bf{0} \}, \\[0.3em]
\text{im}\, (\bf{A}) &= \{\bf{Av}: \bf{v}\in \F^n\}, \\[0.3em]
\text{rank}\, (\bf{A}) &= \dim (\text{im}\, (\bf{A})), \\[0.3em]
\text{nullity}\, (\bf{A}) & = \dim (\ker (\bf{A})).
\end{aligned}
$$
:::
:::
...
$\bf{A}\in \F^{m \times n}$ 에 대해 $\ker (\bf{A})$ 는 $\F^n$ 의 부분공간이며 $\text{im}\, (\bf{A})$ 는 $\F^{m}$ 의 부분공간이다. 또한 다음이 성립한다.

$$
\dim (\F^m) = \text{rank}\, (\bf{A}) + \text{nullity}\, (\bf{A}).
$$
...
행렬식 $\det :\F^{n \times n} \to \mathbb{F}$ 은 다음과 같이 정의된 함수이다. [@eq-determinant_of_matrix 에 관한 사항들은 선형대수학 책을 참고하라]{.aside}

$$
\det(\bf{A}) = \sum_{\sigma \in S_n} \text{sgn}(\sigma)\left(\prod_{i=1}^n A_{i\sigma(i)}\right)
...
$\bf{A}\in \F^{n \times n}$ 와 $c\in \mathbb{F}$ 에 대해 다음이 성립한다.

&emsp; ($1$) $\bf{A}$ 의 서로 다른 두 행이나 두 열의 위치를 바꾼 행렬을 $\bf{A}_1$ 이라 할 때 $\det(\bf{A}_1) = -\det (\bf{A})$ 이다.

&emsp; ($2$) $\bf{A}$ 의 한 행 혹은 한 열에 상수 $c$ 가 곱해진 행렬을 $\bf{A}_2$ 라 할 때 $\det(\bf{A}_2) = c\det(\bf{A})$ 이다.

&emsp; ($3$) $\bf{A}$ 의 한 행이 다른 행의 상수곱이거나 한 열이 다른 열의 상수곱이면 $\det (\bf{A}) = 0$ 이다.

&emsp; ($4$) $\det(\bf{A}^T) = \det(\bf{A})$ 이다.

&emsp; ($5$) $\det(\bf{AB}) = \det(\bf{A}) \det(\bf{B})$ 이다.
...
$n \times n$ 행렬을 **정사각 행렬(square matrix)** 이라고 한다. $\bf{A}\in \F^{n \times n}$ 가 $A_{ii}=1$, $(i=1,\ldots,\,n)$ 이며 나머지 성분이 $0$ 일 때 **항등 행렬 (Identity matrix)** 라고 하며 $\bf{I}_n$ 으로 표기한다. 행렬의 크기를 구체적으로 표기할 필요가 없을 때는 $\bf{I}$ 라고 표기한다.

$\bf{X}\in \F^{n\times n}$ 에 대해 어떤 행렬 $\bf{Y}\in \F^{n\times n}$ 가 존재하여 $\bf{XY}=\bf{YX}=\bf{I}_n$ 일 때 $\bf{Y}$ 를 $\bf{X}$ 의 **역행렬 (inverse matrix)** 라고 하며 행렬 $\bf{A}$ 의 역행렬은 $\bf{A}^{-1}$ 로 표기한다. 역행렬이 존재하는 행렬을 **가역 행렬** 이라고 한다.
:::
::: 
...
$\bf{A},\,\bf{B}\in \F^{n\times n}$ 와 $c\in \mathbb{F}$ 에 대해 다음이 성립한다.

&emsp; ($1$) $(\bf{A}^{-1})^{-1} =\bf{A}$,

&emsp; ($2$) $(c\bf{A})^{-1} = \dfrac{1}{c} \bf{A}^{-1}$,

&emsp; ($3$) $(\bf{AB}^{-1}) =\bf{B}^{-1}\bf{A}^{-1}$.

&emsp; ($4$) $\det(\bf{A}^{-1}) = \dfrac{1}{\det (\bf{A})}$
:::
...
정사각 행렬 $\bf{A}$ 가 가역행렬일 필요충분조건은 $\det (\bf{A}) \ne 0$ 이다.
:::
...
$\bf{A} \in \F^{m\times n}$ 과 $\bf{x}\in \F^n$, $\bf{y} \in \F^m$, 그리고 가역행렬 $\bf{S}\in \F^{m \times m}$ 에 대해 다음이 성립한다.

$$
\bf{Ax}=\bf{y} \iff \bf{SAx} = \bf{Sy}
$$
...
$\{\bf{v}_1,\ldots,\,\bf{v}_n \}$ 가 $\F^n$ 의 기저이며 $\bf{P}\in \F^{n \times n}$ 가 가역이면 $\{\bf{Pv}_i : i=1,\ldots,\,n\}$ 도 $\F^n$ 의 기저이다.
...
두 정사각 행렬 $\bf{A},\, \bf{B}\in  \F^{n \times n}$ 에 대해 어떤 가역 행렬 $\bf{P}\in \F^{n \times n}$ 가 존재하여 $\bf{A}= \bf{PBP}^{-1}$ 일 때 두 행렬을 **닮은 행렬** 이라고 한다. 행렬 $\bf{A}$ 와 가역행렬 $\bf{P}$ 에 대해 $\bf{PAP}^{-1}$ 로 바꾸는 것을 **닮음 변환 (similar transform)** 이라고 한다.
...
닮음 변환은 기저의 변환이다. $\mathcal{B} = \{u_1,\ldots,\,u_n\}$, 가 벡터공간 $U$ 의 기저라고 하고 $T\in \mathcal{L}(U)$ 에 대해 $[T]_{\mathcal{B}} = \bf{A}$ 라고 하자. $[u_i]_{\mathcal{B}}= \hat{\bf{e}}_i$ 이다. $U$ 의 다른 기저 $\mathcal{B}' = \{u_1', \ldots,\, u_n'\}$ 에 대해 $\bf{p}_i = [u'_i]_{\mathcal{B}}$ 를 $i$ 번째 열벡터로 갖는 행렬을 $\bf{P}$ 라고 하자. 이 때 $\bf{PAP}^{-1} = [T]_{\mathcal{B}'}$ 이다. 
...
#### 고유값, 고유벡터, 고유공간
행렬 $\bf{A}\in \F^{m \times n}$ 에 대해 $\bf{Av}=\lambda \bf{v}$ 를 만족하는 $\lambda \in \mathbb{F}$ 와 $\bf{0}$ 이 아닌 $\bf{v}\in \F^n$ 가 존재할 때 $\lambda$ 를 $\bf{A}$ 의 **고유값 (eigen value)** 이라고 하고 $\bf{v}$ 를 고유값 $\lambda$ 에 대한 $\bf{A}$ 의 **고유 벡터 (eigne vector)** 라고 한다. 고유값 $\lambda$ 를 갖는 고유벡터의 집합과 $\{\bf{0}\}$ 의 합집합을 고유값 $\lambda$ 에 대해 $\bf{A}$ 의 **고유 공간 (eigenspace)** 라고 하며, $E(\lambda, \bf{A})$ 라고 표기한다.
...
정사각 행렬 $\bf{A}\in \F^{n \times n}$ 에 대해 $p_\bf{A}(\lambda) = \det(\lambda \bf{I}_n -\bf{A})$ 는 $\lambda$ 에 대한 $n$-차 다항식으로 행렬 $\bf{A}$ 의 **특성다항식 (characteristic polynomial)** 이라 한다. 
...
$\lambda \in \mathbb{F}$ 가 $\bf{A}\in \F^{n\times n}$ 의 고유값이기 위한 필요충분조건은 $p_{\bf{A}}(\lambda) =  0$ 인 것이다. 
...
복소수체에서 정의된 정사각 행렬 $\bf{A}\in \C^{n\times n}$ 에 대한 고유값이 항상 존재한다.
...
$\mathcal{B}=\{v_1,\ldots,\,v_n\}$ 이 내적벡터공간 $V$ 의 기저이며 $\langle v_i,\,v_j\rangle = \delta_{ij}$ 일 때 $\mathcal{B}$ 를 $V$ 의 **정규직교기저 (orthonormal basis)** 라고 한다. 벡터공간 $\F^n$ 에 대해 $\hat{\bf{e}}_i \in \F^n$ 를 $(\hat{\bf{e}}_i)_j = \delta_{ij}$ 로 정의하자. 이 때 $\{\hat{\bf{e}}_1, \ldots, \hat{\bf{e}}_n\}$ 는 $\F^n$ 의 정규직교기저이며 특히 **표준 기저 (standard basis)** 라고 한다.
...
$\mathbb{R}^n$ 벡터공간에 대해 내적이

$$
\langle \bf{x},\, \bf{y} \rangle = \sum_{i=1}^n x_i y_i = \bf{x}^T\bf{y}
$$
...
$\bf{e}_i \in \mathbb{R}^n$ 을 $i$ 번째 성분만 $1$ 이며 나머지 성분은 $0$ 인 유클리드 공간의 벡터라고 하자. 이 때 $\mathcal{B}_S = \{\bf{e}_1,\ldots,\,\bf{e}_n\}$ 을 유클리드 공간의 **표준기저 (standard basis)** 라고 한다.
...
$\mathbb{R}$-내적공간 $V$ 의 정규직교 기저 $\mathcal{B}_V$ 에 대한 벡터 $u, v$ 의 행렬표현 $\bf{u} = [u]_{\mathcal{B}_V},\, \bf{v} = [v]_{\mathcal{B}_V}$ 에 대해 $\langle u, v\rangle = \bf{v}^T\bf{u}$ 이다. $V$ 가 $\mathbb{C}$-내적공간일 경우 $\langle u, v\rangle = \bf{v}^\ast\bf{u}$ 이다.
...
$\bf{u},\,\bf{v}\in \F^n$, $\bf{A} \in \F^{n \times n}$ 와 $\bf{A}$ 의 수반행렬 $\bf{A}^\ast$ 에 대해 다음이 성립한다.

$$
\langle \bf{Au}, \bf{v}\rangle = \langle \bf{u}, \bf{A}^\ast \bf{v}\rangle
$$
...
$\bf{S}\in \F^{n\times n}$ 가 모든 $\bf{v}\in \F^n$ 에 대해 $\|\bf{Sv}\| = \|\bf{v}\|$ 이면 $\bf{S}$ 를 **Isometry** 라고 한다. $\mathbb{F} =\mathbb{R}$ 일 때 **직교 행렬(orthogonal matrix)** 이라고 하며 $\mathbb{F}=\mathbb{C}$ 일 때 **유니타리 행렬(unitary matrix)** 라고 한다. 직교 행렬에 의한 닮음변환을 **직교 변환 (orthogonal transformation)** 이라고 하며, 유니타리 행렬에 의한 닮음변환을 **유니타리 변환 (unitary transformation)** 이라고 한다.
:::
:::
...
$\bf{S}$ 가 Isometry 일 때 다음이 성립한다.

&emsp; ($1$) $\{\bf{v}_1,\ldots,\bf{v}_n\}$ 이 정규직교기저일 때 $\{\bf{Sv}_1,\ldots,\bf{Sv}_n\}$ 도 정규직교기저이다.

&emsp; ($2$) $\bf{S}^{-1}= \bf{S}^\ast$ 이다. 따라서 $\bf{O}$ 가 직교행렬이면 $\bf{O}^{-1}=\bf{O}$ 이다. 
...
정사각 행렬 $\bf{A} \in \F^{n \times n}$ 가 $\bf{AA}^\ast = \bf{A}^\ast\bf{A}$ 일 때 이 행렬을 **정규 행렬 (normal matrix)** 라고 한다.
...
$\bf{A}\in \F^{n \times n}$ 가 정규행렬일 때 다음이 성립한다.

&emsp; ($1$) $\bf{A}$ 가 정규행렬일 필요충분조건은 모든 $\bf{v}\in \F^n$ 에 대해 $\|\bf{Av}\|=\|\bf{A}^\ast\bf{v}\|$ 이다. 

&emsp; ($2$) $\bf{v}$ 가 공유값 $\lambda$ 를 갖는 $\bf{A}$ 의 고유벡터이면 고유값 $\overline{\lambda}$ 를 갖는 $\bf{A}^\ast$ 의 고유벡터이다.

&emsp; ($3$) 서로 다른 고유값을 갖는 $\bf{A}$ 의 고유벡터는 직교한다.

&emsp; ($4$) $\bf{A}$ 가 복소행렬일 때 상삼각행렬 꼴은 대각행렬이다.$^\ast$[$^\ast$ 슈어 정리 에 의해 항상 상삼각 행렬 꼴로 표현할 수 있음을 안다.]{.aside}
...
$\bf{A}\in \C^{n \times n}$ 일 때 $\bf{A}^\ast = \bf{A}$ 인 행렬을 에르미트 행렬이라고 한다. 에르미트 행렬은 정규행렬이다. 에르미트 행렬에 대해 다음이 성립한다.
...
$\bf{A}\in \R^{n \times n}$ 일 때 $\bf{A}^T= \bf{A}$ 인 행렬을 대칭 행렬이라고 한다. 대칭행렬은 정규행렬이다. 대칭행렬에 대해 다음이 성립한다.
...
$\mathbb{R}^n$ 에서의 열린 집합 $U$ 에서 정의된 함수 $F:U \to \mathbb{R}^m$ 이 

$$
\lim_{\bf{h} \to \bf{0}} \|F(\bf{p} + \bf{h}) - F(\bf{p})\|  = 0
$$

일 때 $F$ 는 $\bf{p}$ 에서 **연속** 이라고 한다. $F$ 가 모든 $\bf{p}\in U$ 에서 연속일 때 $F$ 를 $U$ 에서 연속이라고 한다.
...
$\mathbb{R}^n$ 에서의 열린 집합 $U$ 에서 정의된 함수 $F:U \to \mathbb{R}^m$ 이 $\bf{p}\in U$ 에서 어떤 행렬 $M\in \R^{m \times n}$ 에 대해

$$
\lim_{\bf{h} \to \bf{0}} \dfrac{\|F(\bf{p} + \bf{h}) - F(\bf{p}) - \bf{Mp}\|}{\|\bf{h}\|} = \bf{0}
$$

를 만족 할 때 $F$ 는 $\bf{p}$ 에서 미분가능하다 라고 한다. 이때 $\bf{M}$ 을 $\bf{p}$ 에서의 $F$ 의 **자코비 행렬(Jacobian matrix)** 이라고 하며 $DF(\bf{p}),\, \bf{J}_F(\bf{p})$ 옥은 $F'(\bf{p})$ 라고 표기한다. $F$ 가 모든 $\bf{p}\in U$ 에서 미분 가능일 때 $F$ 를 $U$ 에서 미분가능하다 라고 한다.
...
$\mathbb{R}^n$ 에서의 열린 집합 $U$ 에서 정의된 함수 $F:U \to \mathbb{R}^m$ 에 대해 다음은 동치이다.

&emsp; ($1$) $F$ 는 $\bf{p}\in U$ 에서 미분가능하다.

&emsp; ($2$) 어떤 $\bf{M}\in \R^{m \times n}$ 과 $\displaystyle \lim_{\bf{h}\to \bf{0}}\dfrac{R(\bf{h})}{\|\bf{h}\|} = \bf{0}$ 인 어떤 함수 $R:U \to \mathbb{R}^m$ 에 대해 
$$
F(\bf{p}+\bf{h}) = \bf{Mp}+R(\bf{v})
$$ 
&emsp; &emsp; 이다. 
...
$F : U\subset \mathbb{R}^n \to \mathbb{R}^m$ 가 $\bf{p}\in U$ 에서 미분가능할 때 임의의 $\bf{u}\in \mathbb{R}^n-\{\bf{0}\}$ 에 대해 아래의 극한이 존재한다.

$$
\lim_{t \to \infty} \dfrac{F(\bf{p}+t\bf{u}) - F(\bf{p})}{t}
...
#### 방향미분과 편미분
$\bf{u}$ 가 단위벡터, 즉 $\|\bf{u}\|=1$ 일 때 @eq-directional_derivative 의  극한을 $F$ 의 $\bf{p}$ 에서의 $\bf{u}$ 방향으로의 **방향미분 (directional derivative)** 라고 하고 $D_\bf{u}F(\bf{p})$ 라고 쓴다. $\bf{u}$ 가 어떤 표준기저 $\hat{\bf{e}}_i$ 일 때의 방향미분을 **편미분 (partial derivative)** 라고 하며 $D_{\hat{\bf{e}}_i} F(\bf{p})$ 를 $D_i F(\bf{p})$, $\dfrac{\partial F}{\partial x_i}(\bf{p})$, 혹은 $\partial_i F(\bf{p})$ 라고 표기한다. 
...
$F : U \subset \mathbb{R}^n \to \mathbb{R}^m$ 이 $\bf{p}\in U$ 에서 미분가능이면 $\bf{p}$ 에서 연속이다.
...
$\mathbb{R}^{5 \times 1}$ 의 단위행렬 $\bf{e}_3$ 는 다음과 같이 얻을 수 있다.

```julia
e_3 = Matrix{Float64}(I, 5, 5)[:,3]
```

**(M-6)** $m\times n$ 행렬 $\bf{A}$ 와 $m \times k$ 행렬 $\bf{B}$ 에 대해(두 행렬의 행의 갯수가 같음에 주의하라)) $[\bf{A}\; \bf{B}]$ 는 $\bf{A}$ 행렬 옆에 $\bf{B}$ 행렬을 두는 $m \times (n+k)$ 행렬을 의미한다. $m\times n$ 행렬 $\bf{A}$ 와 $l \times n$ 행렬 $\bf{C}$ 에 대해 (두 행렬의 열의 갯수가 동일함에 유의하라) $[\bf{A} \,; \bf{C}]$ 는 $\bf{A}$ 행렬 아래에 $\bf{C}$ 행렬을 두는 $(m+l)\times n$ 행렬이다. 아마 $\begin{bmatrix} \bf{A} \\ \bf{C} \end{bmatrix}$ 와 동일하다고 하면 이해하기 쉬울 것이다. `;` 기호는 julia 에서 열바뀜을 나타내므로 julia 와 어느정도 유사성이 있다 하겠다. 여기에 $l\times k$ 행렬 $\bf{D}$ 에 대해 $\begin{bmatrix} \bf{A} & \bf{B} \\ \bf{C} & \bf{D} \end{bmatrix}$ 가 의미하는 것은 여러분이 쉽게 유추 할 수 있을 것이다.
...
**(M-7)** $m\times n$ 행렬 $\bf{A}$ 와 $m \times k$ 행렬 $\bf{B}$ 혹은 $m$ 차원 벡터 $\bf{b}$ 에 대해 $[\bf{A} \mid \bf{B}]$ 혹은 $[\bf{A} \mid \bf{b}]$ 는 행렬에 대한 연산을 동시에 적용하기 위한 개념이다. 연산 $\hat{L}[\bf{A} \mid \bf{B}] = [\hat{L}\bf{A} \mid \hat{L}\bf{B}]$ 를 의미한다.
